...
const Gallery = () => {
  ...
  const previousClick = () => {
    setSelectedPhotoLeft(
      (selectedPhotoLeft - 1 + photos.length) % photos.length
    );
    setSelectedPhotoCenter(
      (selectedPhotoCenter - 1 + photos.length) % photos.length
    );
    setSelectedPhotoRight(
      (selectedPhotoRight - 1 + photos.length) % photos.length
    );
  };
  ...
  return (
    <HStack justify={"center"} id="gallery">
      ...
      <HStack spacing={4} justify={"center"}>
        ...
        <Image
          src={photos[selectedPhotoCenter]}
          maxW={"30%"}
          borderRadius="lg"
        />
        <Image
          src={photos[selectedPhotoRight]}
          maxW={"30%"}
          borderRadius="lg"
        />
      </HStack>
      <Box boxSize={20} pr={22}>
        <Button onClick={nextClick}>
          <Icon as={GrNext} boxSize={10} color={"white"}>
            {" "}
          </Icon>
        </Button>
      </Box>
